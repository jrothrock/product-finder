...
class LanguageUtils(object):
    ...
    def get_important_title_words(title, description):
        high_frequency_description = LanguageUtils().get_high_frequency_nouns(
            description
        )
    ...
    def get_weight_or_material(description):
        ...
        material_regex = re.search(
            "(wood|steel|plastic|fabric|polyester)", description, re.IGNORECASE
        )
        if material_regex:
            return {
                "weight": None,
                "measurement": None,
                "material": material_regex.group(1),
            }  # may need to search for most common grouping, but works for now
